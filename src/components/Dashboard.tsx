...
const Dashboard: React.FC<DashboardProps> = ({ onLogout, onSwitchAccount }) => {
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [currentView, setCurrentView] = useState<'chat' | 'about' | 'bookmarks' | 'liked' | 'disliked' | 'history' | 'workspaces' | 'data' | 'home'>('chat');
      ...
      <div className={`bg-slate-800 text-white transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-16'} flex flex-col`}>
        ...
        <div className="p-4 border-t border-slate-700">
          <div className="flex items-center space-x-3">
            ...
            {sidebarOpen && (
              <div>
                <div className="font-medium">Sodadasiswaroop User</div>
              </div>
            )}
          </div>
        </div>
